...
class MainProcess(object):
  ...
  file.close() 
  if LIST_S[0].strip()=="0":#25%
     Size=2
  elif LIST_S[0].strip()=="1":#50%
     Size=4
  elif LIST_S[0].strip()=="2":#74%
     Size=6
  elif LIST_S[0].strip()=="3":#100%
     Size=8
  ...
  def crop(self,nameFile):
      ...
      cv2.imwrite('image/picr.png',pic) 
...
m=MainProcess(0)
l=os.listdir("image\input")
for i in range(len(l)):
  print 'image\input\{:>}'.format(l[i])
code,list_a,Set,Term=m.Process('image\input\exam2.png' )
print code,Set,Term"""
